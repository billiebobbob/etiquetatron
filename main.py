...
def print_image(filepath, printer_name=None, tape_mm=62, cut_length_mm=None):
    """Imprime una imagen en la impresora seleccionada.
    tape_mm: ancho de cinta (62mm para Brother QL-800).
    cut_length_mm: largo de corte (152mm envio, 62mm producto). Si None, se calcula del DPI.
    """
    if sys.platform in ('darwin',) or sys.platform.startswith('linux'):
        # Rotar imagen a portrait si es landscape, para coincidir con la cinta
        ...
        print_path = rotated_path or filepath
        ...
        cmd.extend(['-o', f'media=Custom.{tape_mm}x{cut_length_mm}mm'])
        cmd.extend(['-o', 'fit-to-page'])
        cmd.append(print_path)
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)

        # Limpiar archivo temporal rotado
        if rotated_path:
            try:
    ...
    def print_all_from_folder(self):
        ...
        def _print_thread():
            ...
            for i, fname in enumerate(files):
                ...
                try:
                    result = print_image(filepath, printer, tape_mm, cut_length_mm)
                    if result and result.returncode == 0:
                        printed += 1
